...
export const CommentaryList = ({ productName }: { productName: string }) => {
  ...
  const handleCommentaryClick = () => {
    ...
    if (userData) {
      const hasAComment = userData.user.comments.some(
        (comment) => comment.product_name == product?.name
      );
      hasAComment
    ...
    <section className="flex flex-col gap-5">
      <button onClick={() => handleCommentaryClick()}>
        <CommentaryIcon>
          ...
          <p className="border px-2 border-second rounded-md md:hover:translate-y-px transition-all hover:bg-second hover:text-primary">
            Deixe sua Avaliação
          </p>
        </CommentaryIcon>
      </button>
      <ul className="flex flex-col gap-5 divide-y divide-second">
        {!product ? (
